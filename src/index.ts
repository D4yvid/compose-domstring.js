...
export function renderToString<T>(
    ...
    function doRender<T>(instance: ComposableInstance<T>) {
        ...
        if (instance.rawChildren) {
            for (let [_, child] of instance.rawChildren.entries()) {
    ...
    function elementToString(element: Element): string {
        ...
        if (element.attributes && Array.isArray(element.attributes)) {
            for (let attribute of element.attributes) {
                if (attribute.code) {
                    console.warn("Code attribute not handled: ", attribute.key);
                ...
                if (attribute.value && typeof (attribute.value) != 'boolean' && attribute.value !== true) {
